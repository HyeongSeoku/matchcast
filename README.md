...
```bash
brew tap HyeongSeoku/matchcast
brew install matchcast
matchcast
```

Or via npm:
```bash
npm install -g matchcast
matchcast
```

<details>
<summary>Run from source (for development)</summary>

```bash
git clone https://github.com/HyeongSeoku/matchcast.git
cd matchcast
npm install
npm run build
node dist/index.js
...
Or run directly with tsx: `npm run dev` / `npm run dev -- list`.
</details>
...
```
┌────────────────────────────────────────────────────────────────────┐
│ Scoreboard — score, clock, competition                             │
├────────────────────────────────────────────────────────────────────┤
│ Ground — venue / attendance / referee                              │
│ possession-shaded pitch, formation drawn on the grid               │
├─────────────────────────────────┬──────────────────────────────────┤
│ Stats · Lineups                 │ Events — commentary feed         │
│ (scrolls)                       │ (scrolls, auto-follows)          │
└─────────────────────────────────┴──────────────────────────────────┘
```
...
<details>
<summary>소스에서 직접 실행 (개발용)</summary>
...
개발 중에는 `npm run dev` / `npm run dev -- list` 처럼 tsx로 바로 실행할 수도 있습니다.
</details>
...
matchcast live <leagueSlug> <eventId>
# 예: matchcast live fifa.world 760507
```
...
├────────────────────────────────────────────────────────────────────┤
│ 그라운드 — 경기장 / 관중 / 심판                                    │
│ 점유율 배경 + 그리드 위에 그린 포메이션                            │
├─────────────────────────────────┬──────────────────────────────────┤
│ 통계 · 라인업                   │ 이벤트 — 코멘터리 피드           │
│ (스크롤)                        │ (스크롤, 자동으로 맨 아래 따라감)│
└─────────────────────────────────┴──────────────────────────────────┘
```
